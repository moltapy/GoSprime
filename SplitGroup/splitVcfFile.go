...
func splitVcfFile(subPop string, popLogChan chan string) {

	vcfLogChan := make(chan string, 22)

	popLogInfo := "Start processing bcftools concat " + subPop + " with " + *args.OutGroup
	popLogChan <- popLogInfo
	...
	for chrom := 1; chrom <= 22; chrom++ {
		...
		waitBcfGroup.Add(1)
		go bcftoolExec(*args.BcfTool, subPop, vcfFile, outFile, sampleFile, vcfLogChan)
	}
	waitBcfGroup.Wait()
	// consider range

	for info := range vcfLogChan {
		//log.Print(info)
		popLogChan <- info
	}
